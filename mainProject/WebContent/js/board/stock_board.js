		...
		request.onload = function () {
			...
			for (var i = 0; i < detail.replys.length; i++) {
				contentSum += '<div><span class="re-writer">'
					+ detail.replys[i].writerId + ' : </span><span class="re-content">'
					+ detail.replys[i].reContent + '</span><input type="text" class="reply-modi-content hidden" name="title" maxlength="200" placeholder="주제와 무관한 댓글, 악플은 징계 대상이 됩니다." value="'
					+ detail.replys[i].reContent + '"></input><span class="modi-box"><a href="" class="re-modi" data-id="'
					+ detail.replys[i].replyId + '">수정</a>  <a href="" class="re-del" data-id="'
					+ detail.replys[i].replyId + '">삭제</a><a href="" class="re-commit hidden" data-id="'
					+ detail.replys[i].replyId + '">확인</a>  <a href="" class="re-cancel hidden" data-id="'
					+ detail.replys[i].replyId + '">취소</a></span></div>';
			}
